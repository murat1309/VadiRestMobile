...
/*		String sql = "SELECT IB.ID,IB.IHR1PERSONEL_ID,IB.NHR3HESAPDONEM_ID,IB.MHR3HESAPTURU_ID,IB.YILI,IB.AY,IB.SIRA "
		...
		String sql = "select ID, IHR1PERSONEL_ID, HESAPDONEM_ID, YILI, AY, SIRA, UPDUSER, UPDDATE, CRUSER, BRUTTUTAR, NETTUTAR, ODENENTUTAR, KESINTITUTARI, KADRO, GOREV, ODEMETARIHI, MAASTANIM FROM (select DISTINCT IB.ID as ID, IB.IHR1PERSONEL_ID as IHR1PERSONEL_ID, NHR3HESAPDONEM_ID as HESAPDONEM_ID, IB.YILI, IB.AY, IB.SIRA, IB.UPDUSER,IB.UPDDATE, IB.CRUSER, IB.CRDATE,IB.BRUTTUTAR,IB.NETTUTAR as NETTUTAR,IB.ODENENTUTAR,IB.KESINTITUTARI,(SELECT TANIM FROM BSM2SERVIS WHERE ID=IP.BSM2SERVIS_KADRO) AS KADRO,(SELECT TANIM FROM BSM2SERVIS WHERE ID=IP.BSM2SERVIS_GOREV) AS GOREV,(SELECT ODEMETARIHI FROM NHR3HESAPDONEM WHERE ID=IB.NHR3HESAPDONEM_ID) ODEMETARIHI,(SELECT TANIM FROM NHR3HESAPDONEM WHERE ID=IB.NHR3HESAPDONEM_ID) MAASTANIM FROM GHR3HESAP IB, IHR1PERSONEL IP WHERE IB.IHR1PERSONEL_ID = IP.ID AND IB.YILI = " + year + " AND EXISTS (SELECT 1 FROM NHR3HESAPDONEM WHERE ID=IB.NHR3HESAPDONEM_ID AND ODEMETARIHI IS NOT NULL ) UNION select DISTINCT IB2.ID as ID, IB2.IHR1PERSONEL_ID as IHR1PERSONEL_ID, RHR4HESAPDONEM_ID as HESAPDONEM_ID, IB2.YILI, IB2.AY, IB2.SIRA, IB2.UPDUSER, IB2.UPDDATE, IB2.CRUSER, IB2.CRDATE, IB2.BRUTTUTAR,IB2.NET as NETTUTAR,IB2.ODENEN as ODENENTUTAR,IB2.KESINTI as KESINTITUTARI,null as KADRO, null as GOREV, null as ODEMETARIHI,(SELECT TANIM FROM RHR4HESAPDONEM WHERE ID=IB2.RHR4HESAPDONEM_ID) as MAASTANIM FROM HHR4HESAP IB2, IHR1PERSONEL IP WHERE IB2.IHR1PERSONEL_ID = IP.ID AND IB2.YILI = " + year + " AND EXISTS (SELECT 1 FROM RHR4HESAPDONEM WHERE ID=IB2.RHR4HESAPDONEM_ID AND ODEMETARIHI IS NOT NULL )) t WHERE IHR1PERSONEL_ID =" + persid + " ORDER BY YILI DESC, AY DESC ";
